...
import { CheckCircle2, Calendar, FileText, ArrowRight, Bot, Clock } from 'lucide-react'
...
export function VictoryPage() {
  ...
  useEffect(() => {
    confetti({ particleCount: 100, spread: 75, origin: { y: 0.35 }, colors: ['#FBBF24', '#34D399', '#60A5FA', '#A78BFA'] })
    const ts = [
        ...
        {step >= 0 && (
          <div className="relative rounded-2xl border border-amber-500/20 bg-gradient-to-br from-amber-500/8 via-zinc-900 to-emerald-500/5 p-6 text-center overflow-hidden animate-bounce-in">
            <div className="relative z-10">
              ...
              <p className="mt-1 text-sm text-zinc-500">Deal jumped from 68% to 90% health. Close imminent.</p>
              <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
                <span className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 font-mono text-sm font-semibold text-emerald-400">90% Health</span>
                <span className="rounded-full border border-amber-500/30 bg-amber-500/10 px-3 py-1 font-mono text-sm font-semibold text-amber-400">+22 pts</span>
                <span className="rounded-full border border-blue-500/30 bg-blue-500/10 px-3 py-1 text-sm font-semibold text-blue-400">Close this week</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 animate-slide-up">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm font-600 text-zinc-200">Meeting Booked</span>
              </div>
              <CheckCircle2 className="h-4 w-4 text-emerald-400" />
            </div>
            <div className="rounded-xl border border-zinc-800 bg-zinc-950/60 p-3.5">
              <p className="text-xs font-semibold text-zinc-100">{MEETING.title}</p>
              ...
              <div className="mt-2.5 space-y-1">
                <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-600">Auto-generated agenda</p>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 animate-slide-up">
            <div className="mb-3 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-display text-sm font-600 text-zinc-200">DPDP Compliance Sent</span>
              </div>
              <CheckCircle2 className="h-4 w-4 text-emerald-400" />
            </div>
            <div className="mb-3 rounded-lg border border-zinc-800 bg-zinc-950/60 px-3 py-2.5">
              <p className="font-mono text-xs font-semibold text-zinc-300">DPDP_compliance_doc.pdf</p>
              <p className="text-[10px] text-zinc-600 mt-0.5">Sent to 3 recipients</p>
            </div>
            <div className="space-y-2">
              {RECIPIENTS.map(({ contact, status, opened }) => (
                <div key={contact.id} className="flex items-center gap-2.5">
                  <Avatar name={contact.full_name} imageUrl={contact.avatar_url} size="sm" />
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-semibold text-zinc-300">{contact.full_name}</p>
                    <p className="text-[10px] text-zinc-600">{contact.email}</p>
                  </div>
                  <span className={`text-[10px] font-medium ${opened ? 'text-emerald-400' : 'text-zinc-600'}`}>{status}</span>
        ...
        {step >= 3 && (
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5 animate-slide-up">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="font-display text-sm font-600 text-zinc-200">AI Auto-Scheduled Next Steps</span>
            </div>
            <div className="space-y-2">
              {NEXT_STEPS.map((s, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <span className="flex h-4 w-4 shrink-0 items-center justify-center rounded-full bg-zinc-800 border border-zinc-700 font-mono text-[9px] font-semibold text-zinc-500 mt-0.5">{i + 1}</span>
